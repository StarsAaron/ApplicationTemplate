...
import com.aaron.utils.LogUtils;
 ...
 */
public class TaskExecuteQueue extends Thread {
    ...
    public TaskExecuteQueue() {
        ...
        //从线程池中获取
        Executor mExecutorService = ThreadExecutorFactory.getExecutor();
        ...
        while (!quit) {
            try {
                ...
                try {
                    ...
                    LogUtils.i("TaskExecuteQueue", "收到线程中断请求");
                    e.printStackTrace();
                    //被中断的是退出就结束，否则继续
                    if (quit) {
                        TaskItemList.clear();
